...
the catalog's original category. TUI uses the same default and exposes a
`Ctrl+B` visibility toggle. The visibility choice is process-local.
...
disabled or updateable. Direct commands continue to be rejected by the
existing `protected` transaction check.
